...
Particle::Particle(const Point2D &position, const Point2D &velocity) {
...
void Particle::updatePosition(const Rect &boundary) {
    ...
    while (remainingTime > 0) {
        ...
        if (newPosition.getX() < boundary.getPmin().getX()) {
            remainingTime *= (position.getX() - boundary.getPmin().getX()) /
                             (position.getX() - newPosition.getX());
            velocity.setX(-velocity.getX());
            newPosition.setX(boundary.getPmin().getX());
            hitXBoundary = true;
        } else if (newPosition.getX() > boundary.getPmax().getX()) {
            remainingTime *= (boundary.getPmax().getX() - position.getX()) /
                             (newPosition.getX() - position.getX());
            velocity.setX(-velocity.getX());
...
Point2D Particle::getPosition() const {
    return position;
}
