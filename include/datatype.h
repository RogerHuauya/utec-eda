...
public:
    ...
    Safe &operator+=(const U &other) {
        static_assert(std::is_arithmetic<U>::value,
                      "Operation only valid with numeric types");
    ...
    bool operator==(const U &other) const {
        static_assert(std::is_arithmetic<U>::value,
                      "Comparison only valid with numeric types");
    ...
    friend Safe log(const Safe &x) {
        if (x.value <= static_cast<T>(0)) {
            throw std::runtime_error(
                    "Attempted to calculate the logarithm of a non-positive number");
        }
        return Safe(std::log(x.value));
    }
};

// Specialized template functions for mathematical operations
template<typename T>
